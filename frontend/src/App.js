...
class App extends Component {
    ...
    async componentDidMount(){
      ...
      const manager = await hostify.methods.manager().call();
    ...
    isStore = async event => {
      ...
      var rst = await hostify.methods.isStore(this.state.to).call({from:this.state.accounts[0]});
      console.log(rst);
      if (rst == true){
    ...
    async getMyAddress(){
      var ads = await hostify.methods.getMyAddress().call({from:this.state.accounts[0]});
    ...
    getItemInfo = async event => {
      event.preventDefault();
      var rst = await hostify.methods.getItemInfo(this.state.code).call({from:this.state.accounts[0]});
    ...
    setStoreInfo = async event => {
      ...
      await hostify.methods.setStoreInfo(this.state.to).send({from: accounts[0]});
    ...
    createItem = async event => {
      ...
      var rst = await hostify.methods.createItem(this.state.to, this.state.code, this.state.color, todaydate).send({from: accounts[0]});
      console.log(rst);
      if (rst){
    ...
    async getMyItems(){
      var myitems = await hostify.methods.getMyItems().call({from: this.state.accounts[0]});
      this.setState({myitems:myitems});//todo
    }
    
    verifyItems = async event =>{
      event.preventDefault();
      var adbs = await hostify.methods.verifyItems(this.state.to).call({from: this.state.accounts[0]});
      console.log(adbs)
      if (adbs == true){
    ...
    getItemsOwner = async event =>{
      event.preventDefault();
      const accounts = await web3.eth.getAccounts();
      await hostify.methods.getItemOwner(this.state.code).send({from: accounts[0]})
      var ads = await hostify.methods.getItemOwner(this.state.code).call({from: accounts[0]});
    ...
    transferOwnership = async event =>{
      ...
      var rst = await hostify.methods.transferOwnership(this.state.to,this.state.code).send({from: accounts[0]})
      if (flag == true){
            ...
            return(
              <div className="App">
                ...
                <Jumbotron style={bgstyle}>
                  <h1>Hello, you are logged in as seller.</h1>
                  <p>
